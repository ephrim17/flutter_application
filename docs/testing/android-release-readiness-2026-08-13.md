...
| Blocker | The app creates accounts but Settings does not expose complete account deletion; only an unused partial service exists. No privacy-policy or web deletion URL is present in the repository. | Agree retention/multi-church cleanup rules, implement/test deletion, and provide Play Console privacy/deletion URLs. |
